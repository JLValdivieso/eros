...
void Safe_Activate(unsigned int mode){
      ...
      //Unable Safe_Activate trought interruption
        asm volatile ("li t6, (1 << 18)");
        asm volatile ("csrc mie, t6");
    ...
    //Register File
        ...
        //Enable Safe_Activate trought interruption
        asm volatile ("li t6, (1 << 18)");
        asm volatile ("csrs mie, t6");
...
void Safe_Stop(unsigned int master){
  ...
  volatile unsigned int *Safe_config_reg= SAFE_WRAPPER_CTRL_BASEADDRESS;
        ...
        // //Disable Safe_Stop trought interruption
        // asm volatile ("li t6, (1 << 19)");
        // asm volatile ("csrc mie, t6");

        // //Starting Configuration
        // asm volatile("li   t5, %0" : : "i" (SAFE_WRAPPER_CTRL_BASEADDRESS));
        // asm volatile("lw t6, %0(t5)" : : "i" (SAFE_WRAPPER_CTRL_SAFE_CONFIGURATION_REG_OFFSET));
        // asm volatile("beqz t6, _exit_Safe_Stop");

        // asm volatile("sw zero, %0(t5)" : : "i" (SAFE_WRAPPER_CTRL_CRITICAL_SECTION_REG_OFFSET));
        // asm volatile("sw a0, %0(t5)" : : "i" (SAFE_WRAPPER_CTRL_MASTER_CORE_REG_OFFSET));
        // asm volatile("sw zero, %0(t5)" : : "i" (SAFE_WRAPPER_CTRL_SAFE_CONFIGURATION_REG_OFFSET));
        ...
        // //Reference for if-else (already in TMR mode or not checking configuration reg)
        // asm volatile(".global _exit_Safe_Stop");
        // asm volatile("_exit_Safe_Stop:");
...
void handler_host_safe_activate(void){
        asm volatile ("addi sp,sp,-16");     //Store in stack t5, t6
        asm volatile ("sw   t5,12(sp)");
        asm volatile ("sw   t6,8(sp)");
    ...
    //     asm volatile("li   t5, %0" : : "i" (SAFE_WRAPPER_CTRL_BASEADDRESS));
    //     asm volatile("lw t6, %0(t5)" : : "i" (SAFE_WRAPPER_CTRL_SAFE_CONFIGURATION_REG_OFFSET));
    //     asm volatile("bnez t6, _exit_host_safe_activate");
    ...
    //Register File
        ...
        // Reenable Interrupt mstatus(mie)
        asm volatile("li  t6, (1 << 3)");
        asm volatile("csrs mstatus, t6");
        ...
        //Restore current handler interrupt state
        //Set Base Address
        asm volatile("li   t5, %0" : : "i" (SAFE_WRAPPER_CTRL_BASEADDRESS));
        asm volatile("lw        t5,%0(t5)" :: "i" (SAFE_WRAPPER_CTRL_SAFE_COPY_ADDRESS_REG_OFFSET));

        // mstatus
        asm volatile("lw    t6,0(t5)");
        asm volatile("csrw mstatus, t6");
        // mepc
        asm volatile("lw    t6,12(t5)");
        asm volatile("csrw mepc, t6");
        ...
        // //Reference for if-else (already in TMR mode or not checking configuration reg)
        // asm volatile(".global _exit_host_safe_activate");
        // asm volatile("_exit_host_safe_activate:");
...
void handler_host_safe_stop(void){
        asm volatile ("addi sp,sp,-20");     //Store in stack mstatus, mepc and t6
        asm volatile ("sw   t6,8(sp)");

        asm volatile ("csrr t6, mstatus");
        asm volatile ("sw   t6,12(sp)");
        asm volatile ("csrr t6, mepc");
        ...
        // Restore mstatus(mie) and mepc
        asm volatile ("lw   t6,12(sp)");
        asm volatile ("csrw mstatus, t6");
        asm volatile ("lw   t6,16(sp)");
        asm volatile ("csrw mepc, t6");

        //Enable Safe_Stop trought interruption
        asm volatile ("li t6, (1 << 19)");
        asm volatile ("csrs mie, t6");
